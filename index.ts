...
const logsDirectory = path.join(process.env.LOCALAPPDATA as string, "Solar");
...
winston.createLogger({
  ...
  transports: [
    new winston.transports.File({
      filename: path.join(logsDirectory, "Solar.log"),
    }),
  ],
});
  ...
  try {
    ...
    app.use((req, res) => {
      let text: string = JSON.stringify({
        status: 404,
        errorCode: "errors.com.Solar.backend.route.not_found",
        errorMessage:
